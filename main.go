...
func cleanupTestFolder(benchmark *string, directory *string) error {
	if directory != nil && benchmark != nil && *benchmark != "" {
		if dirInfo, err := os.Stat(*directory); err == nil && dirInfo.IsDir() {
			pattern := dirInfo.Name() + "/" + *benchmark + ".*"
			files, err := filepath.Glob(pattern)
			if err != nil {
				return fmt.Errorf("cannot acquire files to delete based on pattern '%s'. error detail '%s'", pattern, err)
			}
			for _, f := range files {
				if err := os.Remove(f); err != nil {
					log.Printf("Cannot remove file '%s'. Error detail '%s'", f, err)
				}
			}
		} else {
			log.Printf("The path '%s' is not a directory. Cannot perform cleanup", *directory)
		}
		log.Printf("The folder '%s' is cleaned-up of files for benchmark named '%s'", *directory, *benchmark)
	}
	return nil
}

func main() {
	defaultDirectory := os.Getenv("TEST_FOLDER_PATH")
	if defaultDirectory == "" {
	...
	go func() {
		...
		for {
			...
			if err := fioCommand.Wait(); err != nil {
				...
				if err = cleanupTestFolder(benchmark, directory); err != nil {
