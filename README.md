Simple shell script for test of NFS performance using [FIO](https://fio.readthedocs.io/en/latest/fio_doc.html) tool (Shell + Docker + Helm)

# NFS Tester using Fio Benchmark Exporter
...
```shell
helm install nfs-tester helm-chart/ --values helm-chart/values.yaml

helm uninstall nfs-tester
```
...
```
./nfs-tester -benchmark=custom -customBenchmarkFioFlags="--name=latency --status-interval=30 --numjobs=1 --ioengine=libaio --direct=1 --bs=4k --iodepth=1 --readwrite=randrw --directory=/tmp --size=1G --runtime=60 --time_based --lat_percentiles=1 --clat_percentiles=0 --group_reporting"
```
